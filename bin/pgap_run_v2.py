...
for sample_id in pgdirlist:
    subprocess.run('${PGAP_INPUT_DIR}./pgap.py -n --no-internet --ignore-all-errors --taxcheck-only -D singularity -o ' + sample_id+'_results '+ pg_dir +'/'+ sample_id+'/input_'+ sample_id + '.yaml', shell = True, check = True)
