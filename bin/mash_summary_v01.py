...
import argparse
"""
This module parses Mash results to an excel sheet and  generates giving genus, species, distance and accession number
"""
...
def df_to_csv(df):
    ...
    df.to_csv('mash_MSreport_tab.txt', index=None, sep='\t', mode='a')
    return
       
# Testing the module
def main():
    with open(sample_path) as samp:content =samp.readlines()
    sample_list= [ x.replace("\n", "") for x in content]
    ...
    for s in sample_list:
            ...
            pre_result= mash_report(path_to_file,s) # Function 1 produces len(sample_list) lists
            list_of_results= result_list(pre_result) # function 2 making a list of  all lists
    
    datafr = result_to_df(list_list, saved_path)# Function 4 making a data frame with  list of lists
    datafr1 = result_to_df(list_of_results, saved_path) # replaces lsit_list.appenf()
    df_to_csv(datafr1)
    #print(datafr)
    #print(datafr1)
