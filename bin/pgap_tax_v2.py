...
parser = argparse.ArgumentParser(prog = 'pgap_tax_v2.py [Options]')
...
for sample_id in pgdirlist:
    #subprocess.run('${PGAP_INPUT_DIR}./pgap.py -n --no-internet --ignore-all-errors --container-path ${PGAP_INPUT_DIR}pgap_* --taxcheck-only -o ' + sample_id+'_taxCheck '+ pg_dir +'/'+ sample_id+'/input_'+ sample_id+'.yaml', shell = True, check = True)
    subprocess.run('${PGAP_INPUT_DIR}./pgap.py -n --ignore-all-errors -D singularity --taxcheck-only -o ' + sample_id+'_taxCheck '+ pg_dir +'/'+ sample_id+'/input_'+ sample_id+'.yaml', shell = True, check = True)
